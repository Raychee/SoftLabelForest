...
Data(_COMP_T* x, _SUPV_T* y,
     ...
     _N_DAT_T* index, _N_DAT_T _num_of_samples, bool copy_index)
      : buffer(NULL),
        managed_index(NULL),
        num_of_samples_(0),
        labels(NULL) {
    load(x, y, dimension, total_samples, index, _num_of_samples, copy_index);
}
...
template <typename _COMP_T,
          ...
          typename _N_DAT_T>
Data<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>::
Data(const Data& some, _N_DAT_T* index, _N_DAT_T _num_of_samples, bool copy_index)
      : buffer(NULL),
        managed_index(NULL),
        num_of_samples_(0),
        labels(NULL) {
    load(some, index, _num_of_samples, copy_index);
}
...
load(_COMP_T* x, _SUPV_T* y,
    ...
    if (index != NULL && _num_of_samples > 0) {
        if (copy_index) {
            managed_index = new _N_DAT_T[_num_of_samples];
            std::memcpy(managed_index, index, sizeof(_N_DAT_T) * num_of_samples_);
        } else {
            managed_index = index;
        }
        num_of_samples_ = _num_of_samples;
    } else {
        managed_index = new _N_DAT_T[total_samples];
        num_of_samples_ = total_samples;
        for (_N_DAT_T i = 0; i < num_of_samples_; ++i) {
            managed_index[i] = i;
        }
    }
    return *this;
...
Data<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>&
Data<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>::
load(const Data& some, _N_DAT_T* index, _N_DAT_T _num_of_samples, bool copy_index) {
    if (&some == this) {
        std::cerr << "**ERROR** -> Data::load() -> load itself." << std::endl;
        return *this;
    }
    clear();
    buffer = some.buffer;
    if (buffer != NULL) {
        ...
        if (index != NULL && _num_of_samples > 0) {
            ...
            num_of_samples_ = _num_of_samples;
            for (_N_DAT_T i = 0; i < num_of_samples_; ++i) {
                managed_index[i] = some.managed_index[index[i]];
            }
        } else {
            managed_index = new _N_DAT_T[some.num_of_samples_];
            num_of_samples_ = some.num_of_samples_;
            std::memcpy(managed_index, some.managed_index,
                        sizeof(_N_DAT_T) * num_of_samples_);
        }
    }
    return *this;
}
...
_load(bpy::object& x, bpy::object& y) {
    ...
    num_of_samples_ = buffer->num_of_samples();
    managed_index = new _N_DAT_T[num_of_samples_];
    for (_N_DAT_T i = 0; i < num_of_samples_; ++i) {
        managed_index[i] = i;
    }
    return *this;
...
Data<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>&
Data<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>::
_load(const Data& some, const bpy::object& bpy_indices) {
    if (&some == this) {
        std::cerr << "**ERROR** -> Data::load() -> load itself." << std::endl;
        return *this;
    }
    ...
    if (buffer != NULL) {
        ...
        num_of_samples_ = (ndim == 1 ?
                           shape[0] : (shape[0] > shape[1] ?
                                       shape[0] : shape[1]));
        managed_index = new _N_DAT_T[num_of_samples_];
        for (_N_DAT_T i = 0; i < num_of_samples_; ++i) {
            managed_index[i] = some.managed_index[indices[i]];
        }
...
_indices() const {
    return wrap_1d_numpy_array(managed_index, num_of_samples_);
}
...
_labels() {
    if (labels == NULL) labels = new Labels(*this);
    return wrap_1d_numpy_array(labels->label, labels->num_of_labels);
}
...
_num_of_sample_of_each_label() {
    if (labels == NULL) labels = new Labels(*this);
    return wrap_1d_numpy_array(labels->num_of_samples_with_label, labels->num_of_labels);
}
...
buffer() const {
    if (pyobj_x == NULL || pyobj_y == NULL) return bpy::tuple();
